#!/usr/bin/env node
// Removes deploy/Docker artifacts for apps that no longer exist in the workspace.
//
// For each known app Dockerfile (deploy/docker/<app>.Dockerfile):
//   - If apps/<app> does not exist, delete the Dockerfile and remove the
//     corresponding service block from compose.prod.yaml.
//
// Usage:
//   node cleanup-deploy-artifacts.mjs --project-root=<path>
...
const dockerfilesDir = join(projectRoot, 'deploy', 'docker');
const composePath = join(projectRoot, 'deploy', 'compose', 'compose.prod.yaml');
...
for (const filename of dockerfiles) {
	...
	if (!existsSync(appDir)) {
		...
		console.log(`Removed Dockerfile: ${dockerfilePath}`);
