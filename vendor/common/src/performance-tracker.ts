...
export type PerformanceTrackerCallbacks<T> = {
	onStart?: undefined | ((arg: { name: string; startTs: number }) => void);
	onEnd?:
		| undefined
		| ((arg: {
				name: string;
...
export class PerformanceTracker<T> {
	...
	public static hrTimeToMs(hrtime: [number, number]): number {
	...
	public async run<O extends AnyFunction>(
		...
		const duration = PerformanceTracker.hrTimeToMs(end);
