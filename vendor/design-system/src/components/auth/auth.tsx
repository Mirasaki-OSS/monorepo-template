...
export function Auth({
	...
	for (const plugin of plugins) {
		...
		const views = plugin.views as
			| { auth?: Record<string, ComponentType<AuthProps>> }
			| undefined;

		const PluginView = views?.auth?.[pluginView];
		if (!PluginView) continue;
	...
	if (authView === 'signIn' && !emailAndPassword?.enabled) {
		const castPlugins = plugins as (AuthPluginBase & {
			fallbackViews?: { auth?: Record<string, ComponentType<AuthProps>> };
		})[];
		const Fallback = castPlugins.find(
