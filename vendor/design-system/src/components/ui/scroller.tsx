...
const scrollerVariants = cva('', {
	variants: {
		...
		hideScrollbar: {
			true: '[-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden no-scrollbar',
			false: '',
		},
	},
	defaultVariants: {
...
const scrollButtonVariants = cva(
	'absolute z-10 transition-opacity [&>svg]:size-4 [&>svg]:opacity-80 hover:[&>svg]:opacity-100',
	{
		variants: {
			direction: {
				up: 'top-2 start-1/2 -translate-x-1/2 rtl:translate-x-1/2',
				down: 'bottom-2 start-1/2 -translate-x-1/2 rtl:translate-x-1/2',
				left: 'top-1/2 start-2 -translate-y-1/2',
				right: 'top-1/2 end-2 -translate-y-1/2',
			},
		},
		defaultVariants: {
			direction: 'up',
