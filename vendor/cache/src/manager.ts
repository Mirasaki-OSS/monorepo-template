...
{
	protected readonly debug: debug.Debugger = debug('md-oss:cache:manager');
	...
	async ttl(key: string): Promise<number | null> {
		return this.cache.ttl(key).then((ttl) => ttl ?? null);
	}
	...
	async mdel(keys: string[]): Promise<[boolean, boolean[]]> {
		this.debug('[MDEL] Deleting cache keys', keys);
	...
	//
	// Keyv-specific Cache operations
	//
